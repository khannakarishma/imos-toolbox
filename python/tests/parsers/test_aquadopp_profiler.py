"""
Tests for Nortek Aquadopp Profiler ADCP parser

Mirrors MATLAB test structure from Parser/aquadoppProfilerParse.m.
Tests binary .prf format files.

To use these tests:
1. Place your Aquadopp Profiler test files in:
   python/tests/parsers/data/aquadopp_profiler/v000/trip_*/
2. Each trip folder should contain at least one .prf binary file.
3. Run: uv run pytest tests/parsers/test_aquadopp_profiler.py -v
...
def discover_test_files():
    """Discover all Aquadopp Profiler test files (.prf)."""
    files = []
    if not TEST_DATA_DIR.exists():
        return files
    for pattern in ("v000/trip_*/*.prf", "**/*.prf"):
...
@pytest.fixture
def parser():
    """Create an AquadoppProfilerParser instance."""
    return AquadoppProfilerParser()
...
class TestAquadoppProfilerParser:
    """Test suite - mirrors MATLAB aquadoppProfilerParse.m."""

    def test_parser_exists(self, parser):
        """Test that parser can be instantiated."""
        assert parser is not None
        assert isinstance(parser, AquadoppProfilerParser)
        assert parser.parser_name == "Aquadopp Profiler"

    def test_basic_parse(self, parser, test_files):
        """Test basic parsing of a .prf binary file."""
        if not test_files:
...
def test_print_test_file_info():
    ...
    print(f"\n{'=' * 70}")
    print("Nortek Aquadopp Profiler ADCP Test Files Configuration")
    print(f"{'=' * 70}")
    ...
    if not test_files:
        ...
        print(f"  - {TEST_DATA_DIR}/v000/trip_*/*.prf")
    else:
