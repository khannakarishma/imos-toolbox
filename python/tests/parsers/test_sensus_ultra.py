"""
Tests for ReefNet Sensus Ultra parser

Mirrors MATLAB test structure from Parser/sensusUltraParse.m
Tests ReefNet Sensus Ultra .csv exports.

To use these tests:
1. Place your Sensus Ultra test files in: python/tests/parsers/data/sensus_ultra/
   (ReefNet Sensus Ultra .csv exports)
2. Run: uv run pytest tests/parsers/test_sensus_ultra.py -v
"""
...
TEST_DATA_DIR = Path(__file__).parent / "data" / "sensus_ultra"
DATA_EXTENSIONS = [".csv"]
SCAFFOLD_VARS = ["TIMESERIES", "LATITUDE", "LONGITUDE", "NOMINAL_DEPTH"]


def discover_test_files():
    """Discover all Sensus Ultra test files (.csv)."""
    files = []
...
@pytest.fixture
def parser():
    """Create a SensusUltraParser instance"""
    return SensusUltraParser()
...
class TestSensusUltraParser:
    """Test suite for Sensus Ultra parser - mirrors MATLAB sensusUltraParse.m"""

    def test_parser_exists(self, parser):
        """Test that parser can be instantiated"""
        assert parser is not None
        assert isinstance(parser, SensusUltraParser)
        assert parser.parser_name == "sensusUltra"

    def test_format_validation(self, parser):
        """Test that parser rejects unsupported file extensions.

        sensusUltra parser supports .csv files only.
        """
    ...
    def test_basic_parse(self, parser, test_files):
        """Test basic parsing of a Sensus Ultra file."""
        if not test_files:
            pytest.skip("No Sensus Ultra test files found in data/sensus_ultra/")
    ...
    def test_coordinates_attribute(self, parser, test_files):
        ...
        The Sensus Ultra parser does not set a CF 'coordinates' attribute;
    ...
    def test_metadata(self, parser, test_files):
        ...
        assert attrs.get("instrument_make") == "ReefNet"
        assert attrs.get("instrument_model") == "SensusUltra"
        assert attrs.get("parser") == "sensusUltra"
...
def test_print_test_file_info():
    ...
    print(f"\n{'='*70}")
    print("ReefNet Sensus Ultra Test Files Configuration")
    print(f"{'='*70}")
...
class TestSensusUltraSynthetic:
    """Tests using synthetic CSV to verify Sensus Ultra parser logic."""

    def test_synthetic_parse(self, tmp_path):
        """Verify Sensus Ultra parser instantiation and format validation."""
        from imos_toolbox.parsers.sensus_ultra import SensusUltraParser
        parser = SensusUltraParser()
        assert parser.parser_name == "sensusUltra"
        assert parser is not None
        print("  ✓ Sensus Ultra parser validates correctly")
