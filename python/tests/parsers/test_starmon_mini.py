"""
Tests for Star-Oddi Starmon Mini parser

Mirrors MATLAB test structure from Parser/staroddiParse.m
Tests Star-Oddi Starmon Mini .dat exports.

To use these tests:
1. Place your Starmon Mini test files in: python/tests/parsers/data/starmon_mini/
   (Star-Oddi .dat exports)
2. Run: uv run pytest tests/parsers/test_starmon_mini.py -v
"""
...
TEST_DATA_DIR = Path(__file__).parent / "data" / "Star_oddi" / "mini"
DATA_EXTENSIONS = [".dat"]
SCAFFOLD_VARS = ["TIMESERIES", "LATITUDE", "LONGITUDE", "NOMINAL_DEPTH"]


def discover_test_files():
    """Discover all Starmon Mini test files (.dat)."""
    files = []
...
@pytest.fixture
def parser():
    """Create a StarmonMiniParser instance"""
    return StarmonMiniParser()
...
class TestStarmonMiniParser:
    """Test suite for Starmon Mini parser - mirrors MATLAB staroddiParse.m"""

    def test_parser_exists(self, parser):
        """Test that parser can be instantiated"""
        assert parser is not None
        assert isinstance(parser, StarmonMiniParser)
        assert parser.parser_name == "StarmonMini"

    def test_format_validation(self, parser):
        """Test that parser rejects unsupported file extensions.

        StarmonMini parser supports .dat files only.
        """
    ...
    def test_basic_parse(self, parser, test_files):
        """Test basic parsing of a Starmon Mini file."""
        if not test_files:
            pytest.skip("No Starmon Mini test files found in data/starmon_mini/")
    ...
    def test_coordinates_attribute(self, parser, test_files):
        ...
        The Starmon parser does not set a CF 'coordinates' attribute;
    ...
    def test_metadata(self, parser, test_files):
        ...
        assert attrs.get("instrument_make") == "Star ODDI"
        assert attrs.get("parser") == "StarmonMini"
...
def test_print_test_file_info():
    ...
    print(f"\n{'='*70}")
    print("Star-Oddi Starmon Mini Test Files Configuration")
    print(f"{'='*70}")
