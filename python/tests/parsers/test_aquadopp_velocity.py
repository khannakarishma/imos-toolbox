"""
Tests for Nortek Aquadopp Velocity (current meter) parser

Mirrors MATLAB test structure from Parser/aquadoppVelocityParse.m.
Tests binary .aqd format files.

To use these tests:
1. Place your Aquadopp Velocity test files in:
   python/tests/parsers/data/aquadopp_velocity/v000/trip_*/
2. Each trip folder should contain at least one .aqd binary file.
3. Run: uv run pytest tests/parsers/test_aquadopp_velocity.py -v
...
TEST_DATA_DIR = Path(__file__).parent / "data" / "Nortek" / "aquadopp_velocity"


def discover_test_files():
    """Discover all Aquadopp Velocity test files (.aqd)."""
    files = []
    if not TEST_DATA_DIR.exists():
        return files
    for pattern in ("v000/trip_*/*.aqd", "**/*.aqd"):
...
@pytest.fixture
def parser():
    """Create an AquadoppVelocityParser instance."""
    return AquadoppVelocityParser()
...
class TestAquadoppVelocityParser:
    """Test suite - mirrors MATLAB aquadoppVelocityParse.m."""

    def test_parser_exists(self, parser):
        """Test that parser can be instantiated."""
        assert parser is not None
        assert isinstance(parser, AquadoppVelocityParser)
        assert parser.parser_name == "Aquadopp Velocity"

    def test_basic_parse(self, parser, test_files):
        """Test basic parsing of a .aqd binary file."""
        if not test_files:
    ...
    def test_dimensions(self, parser, test_files):
        """A current meter has only a TIME dimension (single cell)."""
        if not test_files:
            pytest.skip("No test files found")
        ...
        # No profile dimensions for a current meter.
        assert "DIST_ALONG_BEAMS" not in dataset.dataset.dims
        assert "HEIGHT_ABOVE_SENSOR" not in dataset.dataset.dims
    ...
    def test_metadata_extraction(self, parser, test_files):
        """Test that instrument metadata is extracted; featureType == mode."""
        if not test_files:
            pytest.skip("No test files found")
        ...
        assert attrs["instrument_model"] == "Aquadopp Current Meter"
        # Aquadopp Velocity stores the mode as featureType.
        assert attrs["featureType"] == "timeSeries"

    def test_coordinates_attribute(self, parser, test_files):
        """Test that data variables carry a coordinates attribute."""
        if not test_files:
            pytest.skip("No test files found")
...
def test_print_test_file_info():
    ...
    print(f"\n{'=' * 70}")
    print("Nortek Aquadopp Velocity Test Files Configuration")
    print(f"{'=' * 70}")
    ...
    if not test_files:
        ...
        print(f"  - {TEST_DATA_DIR}/v000/trip_*/*.aqd")
    else:
