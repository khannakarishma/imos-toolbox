"""
Tests for Star-Oddi Starmon DST parser

Mirrors MATLAB test structure from Parser/staroddiParse.m
Tests Star-Oddi Starmon DST .dat exports.

To use these tests:
1. Place your Starmon DST test files in: python/tests/parsers/data/starmon_dst/
   (Star-Oddi DST .dat exports)
2. Run: uv run pytest tests/parsers/test_starmon_dst.py -v
"""
...
TEST_DATA_DIR = Path(__file__).parent / "data" / "Star_oddi" / "dst_ctd"
DATA_EXTENSIONS = [".dat"]
SCAFFOLD_VARS = ["TIMESERIES", "LATITUDE", "LONGITUDE", "NOMINAL_DEPTH"]


def discover_test_files():
    """Discover all Starmon DST test files (.dat)."""
    files = []
...
@pytest.fixture
def parser():
    """Create a StarmonDSTParser instance"""
    return StarmonDSTParser()
...
class TestStarmonDSTParser:
    """Test suite for Starmon DST parser - mirrors MATLAB staroddiParse.m"""

    def test_parser_exists(self, parser):
        """Test that parser can be instantiated"""
        assert parser is not None
        assert isinstance(parser, StarmonDSTParser)
        assert parser.parser_name == "StarmonDST"

    def test_format_validation(self, parser):
        """Test that parser rejects unsupported file extensions.

        StarmonDST parser supports .dat files only.
        """
    ...
    def test_basic_parse(self, parser, test_files):
        """Test basic parsing of a Starmon DST file."""
        if not test_files:
            pytest.skip("No Starmon DST test files found in data/starmon_dst/")
    ...
    def test_metadata(self, parser, test_files):
        ...
        assert attrs.get("parser") == "StarmonDST"
...
def test_print_test_file_info():
    ...
    print(f"\n{'='*70}")
    print("Star-Oddi Starmon DST Test Files Configuration")
    print(f"{'='*70}")
