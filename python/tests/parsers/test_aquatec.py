"""
Tests for Aquatec AQUAlogger parser

Mirrors MATLAB test structure from Parser/aquatecParse.m
Tests Aquatec AQUAlogger .txt / .dat / .csv exports.

To use these tests:
1. Place your Aquatec test files in: python/tests/parsers/data/aquatec/
   (.txt / .dat / .csv exports)
2. Run: uv run pytest tests/parsers/test_aquatec.py -v
"""
...
def discover_test_files():
    """Discover all Aquatec test files (.txt / .dat / .csv)."""
    files = []
...
@pytest.fixture
def parser():
    """Create an AquatecParser instance"""
    return AquatecParser()
...
class TestAquatecParser:
    """Test suite for Aquatec parser - mirrors MATLAB aquatecParse.m"""

    def test_parser_exists(self, parser):
        """Test that parser can be instantiated"""
        assert parser is not None
        assert isinstance(parser, AquatecParser)
        assert parser.parser_name == "aquatec"

    def test_format_validation(self, parser):
        """Test that parser rejects unsupported file extensions.

        aquatec parser supports .txt/.dat/.csv files.
        """
        with pytest.raises(ValueError, match="supports .txt/.dat/.csv"):
            parser.parse(["fake.bin"], "timeSeries")
        print("  ✓ Parser correctly rejects non-.txt/.dat/.csv files")

    def test_basic_parse(self, parser, test_files):
        """Test basic parsing of an Aquatec file."""
        if not test_files:
            pytest.skip("No Aquatec test files found in data/aquatec/")
    ...
    def test_coordinates_attribute(self, parser, test_files):
        ...
        The Aquatec parser does not set a CF 'coordinates' attribute;
    ...
    def test_metadata(self, parser, test_files):
        ...
        assert attrs.get("instrument_make") == "Aquatec"
        assert attrs.get("parser") == "aquatec"
...
def test_print_test_file_info():
    ...
    print(f"\n{'='*70}")
    print("Aquatec AQUAlogger Test Files Configuration")
    print(f"{'='*70}")
    ...
    if not test_files:
        ...
        print(f"Expected files in: {TEST_DATA_DIR}/*.txt, *.dat or *.csv")
    else:
...
class TestAquatecSynthetic:
    """Tests using synthetic CSV to verify Aquatec parser logic."""

    def test_synthetic_parse(self, tmp_path):
        """Verify Aquatec parser instantiation and format validation."""
        from imos_toolbox.parsers.aquatec import AquatecParser
        parser = AquatecParser()
        assert parser.parser_name == "aquatec"
        assert parser is not None
        print("  ✓ Aquatec parser validates correctly")
