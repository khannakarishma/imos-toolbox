"""
Tests for RBR XR series parser

Mirrors MATLAB test structure from Parser/XRParse.m
Tests RBR XR420/XR620 classic .dat exports and Ruskin text exports.

To use these tests:
1. Place your XR test files in: python/tests/parsers/data/xr/
   (classic RBR .dat files, or Ruskin .txt exports)
2. Run: uv run pytest tests/parsers/test_xr.py -v
"""
...
class TestXRParser:
    """Test suite for XR parser - mirrors MATLAB XRParse.m"""

    def test_parser_exists(self, parser):
        """Test that parser can be instantiated"""
        assert parser is not None
        assert isinstance(parser, XRParser)
        assert parser.parser_name == "XR"

    def test_format_validation(self, parser):
        """XR parser does not validate by file extension.

        It inspects the file content (classic RBR header vs Ruskin), so
        there is no extension-based ValueError to assert.
        """
        pytest.skip("XR parser dispatches on file content, not extension")

    def test_basic_parse(self, parser, test_files):
        """Test basic parsing of an XR file."""
        if not test_files:
            pytest.skip("No XR test files found in data/xr/")
    ...
    def test_coordinates_attribute(self, parser, test_files):
        ...
        The XR parser does not set a CF 'coordinates' attribute; verify
        the structural TIME dimensioning of data variables instead.
        """
        if not test_files:
            pytest.skip("No XR test files found in data/xr/")
    ...
    def test_metadata(self, parser, test_files):
        ...
        assert attrs.get("instrument_make") == "RBR"
        assert attrs.get("parser") == "XR"
...
def test_print_test_file_info():
    ...
    print(f"\n{'='*70}")
    print("RBR XR Series Test Files Configuration")
    print(f"{'='*70}")
