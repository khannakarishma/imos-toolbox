"""Nortek Continental ADCP parser implementation.

Parses binary files from Nortek Continental ADCP instruments (.cpr files).
Mirrors MATLAB continentalParse.m implementation as an exact functional port.
...
class ContinentalParser(BaseParser):
    """Parser for Nortek Continental ADCP binary files.

    Mirrors MATLAB continentalParse.m. The Continental velocity profile
    records (Id36) share the same binary layout as AWAC velocity profile
    records, so this parser closely follows the AWAC parser patterns.

    Supports:
    - Binary .cpr format files
    ...
    def parse(self, filenames: Iterable[str | Path], mode: str) -> IMOSDataset:
        ...
        if len(file_list) != 1:
            raise ValueError("Continental parser expects exactly one input file")
        ...
        # Continental velocity profile data lives in Id36
        velocity_data = structures.get("Id36", [])
        if not velocity_data:
            raise ValueError("No Continental velocity data found (Id36)")
    ...
    def _frequency_factor(self, freq: int) -> float:
        ...
        Mirrors MATLAB continentalParse.m: 190 kHz -> 0.2221, 470 kHz -> 0.0945,
        otherwise 0.
        """
        if freq == 190:
            return 0.2221
    ...
    def _calculate_distances(self, user: dict, head: dict) -> np.ndarray:
        ...
        Mirrors the distance calculation in continentalParse.m.
        """
        ...
        # Distance to the middle of each cell
    ...
    def _extract_sensor_data(self, velocity_data: list[dict]) -> tuple[np.ndarray, dict]:
        ...
        # pressure = PressureMSB * 65536 + PressureLSW
    ...
    def _process_velocity_data(
        ...
        Mirrors MATLAB: iCommonTime = ismember(time, timeProc); index-assigns
        into freshly created arrays so unmatched timestamps remain 0.
        """
        n_time = len(time)
    ...
    def _determine_orientation(self, status: np.ndarray) -> int:
        ...
        Mirrors MATLAB: adcpOrientation = mode(bitget(status, 1)).
        1 == downward-looking, 0 == upward-looking.
        """
    ...
    def _create_dataset(
        ...
        """Build the IMOS-compliant dataset (mirrors continentalParse.m)."""
        ...
        # Velocity uses HEIGHT_ABOVE_SENSOR when processed, else DIST_ALONG_BEAMS.
        # Diagnostics (ABSIC/SNR/ERR) always use DIST_ALONG_BEAMS (iDimDiag).
